...
sub MilightDevice_RGBW_Off(@)
{
  ...
  # Store value of brightness before turning off
  # "on" will be of the form "on 50" where 50 is current dimlevel
  # Special case if we are using a group device to switch everything off we want to save brightness_on for each light individually
  MilightDevice_BridgeDevices_Update($hash, "brightness_on_save") if ($hash->{SLOT} eq 'A' && AttrVal($hash->{NAME}, "updateGroupDevices", 0) == 1);
...
>>>>>>> ad0ec6164... Whitespace
  ...
  {
    if (ReadingsVal($hash->{NAME}, "state", "") eq "off") {
      MilightDevice_BridgeDevices_Update($hash, "state,brightness");
    } else {
      MilightDevice_BridgeDevices_Update($hash, "bulk");
    }
  }
...
sub MilightDevice_BridgeDevices_Update(@)
{
  ...
  for (my $i = 0; $i < 4; $i++)
  {
    ...
    {
      if ($rdname eq 'brightness_on_save') {
        if (exists ($device->{READINGS}{'brightness_on'}))
        {
          readingsBulkUpdate($device, 'brightness_on', $hash->{READINGS}{'brightness'}{VAL}, 1);
          Log3 ($hash, 4, "brightness_on: ".$device->{READINGS}{'brightness'}{VAL}." for ".$devname);
        }
      } else {
